...
The workflow first builds candidate combinations at an initial clash radius of ``17.5`` A. Two sites are treated as clashing if their pairwise distance is less than or equal to that radius. The initial list of compatible combinations is generated by ``calc_overlap()``.
